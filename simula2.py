...
# Ajustes para ficar "bonito" e didático:
# 1) Ilustração de entrada em UMA figura: depth representativo + IMU + histograma do depth
# 2) Múltiplas rodadas (seeds) com curva de loss: todas + média + desvio
# 3) Métricas além da matriz de confusão: logloss, brier, switch rate, erro de trajetória (RMSE e erro final)
#
...
RUN_SEEDS = [1, 7, 21, 42, 77]  # pode ajustar
EPOCHS = 30
HIDDEN = 32
LR = 0.02
BATCH_SIZE = 32
...
# - Depth: por frame: media e desvio (2*WINDOW_FRAMES)
...
class MLPBinary:
    ...
    def train(self, X, y, epochs=30, batch_size=32, verbose=False):
        n = X.shape[0]
        hist = []
        ...
        for ep in range(1, epochs + 1):
            ...
            L = binary_logloss(y, p_all.flatten())
            hist.append(L)
...
# =========================
# FIGURA BONITA: ENTRADAS (depth + imu + hist)
# =========================
def plot_input_figure(samples):
    # pega amostra do meio (mais estável)
    sdir = samples[len(samples)//2]
    frames, t, imu, label = load_one_sample(sdir)

    img = frames[len(frames)//2]  # frame representativo
    vmin = np.percentile(img, 2)
    vmax = np.percentile(img, 98)

    plt.figure(figsize=(12, 4))

    # Depth representativo
    plt.subplot(1, 3, 1)
    plt.imshow(img, cmap="gray", vmin=vmin, vmax=vmax)
    plt.title("Frame depth (exemplo)")
    plt.axis("off")
    ...
    plt.title("Histograma do depth")
    plt.xlabel("intensidade normalizada")
    plt.ylabel("frequência")

    plt.suptitle(f"Dados de entrada (amostra {sdir.name}) | label={'direita' if label==1 else 'esquerda'}")
    plt.tight_layout()
    plt.show()
...
# UMA RODADA COMPLETA
# =========================
def run_one_experiment(X, y, seed):
    N = len(y)
    ...
    loss_hist = mlp.train(X_train_n, y_train, epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=False)
    ...
    # matriz confusão (mantida, mas não é a principal)
...
def main():
    ...
    # 1) Figura didática das entradas
    plot_input_figure(samples)

    # 2) Rodadas múltiplas
    results = []
    for sd in RUN_SEEDS:
        print(f"\nRodada seed={sd} ...")
        r = run_one_experiment(X, y, sd)
        results.append(r)
        print(f"  acc={r['acc']:.3f} | f1={r['f1']:.3f} | logloss={r['logloss']:.3f} | "
              f"switch={r['switch_rate']:.3f} | traj_rmse={r['traj_rmse']:.3f} | end_err={r['traj_end_err']:.3f}")

    # 3) Gráfico de loss: todas + média + desvio
    H = np.stack([r["loss_hist"] for r in results], axis=0)
    ...
    plt.title("Treinamento: Loss em múltiplas rodadas")
    plt.grid(True)
    plt.legend()
    plt.show()

    # 4) Escolher melhor por menor logloss
    best = sorted(results, key=lambda d: d["logloss"])[0]
    print(f"\nMelhor rodada: seed={best['seed']} | logloss={best['logloss']:.3f} | acc={best['acc']:.3f}")
    ...
    # 5) Simulação temporal: real vs previsto
    ...
    # 6) Probabilidade ao longo do tempo (controle)
    plt.figure()
    ...
    plt.title("Probabilidade ao longo do tempo")
    plt.grid(True)
    plt.legend()
    plt.show()

    # 7) Trajetória real vs prevista
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title(f"Trajetórias | RMSE={best['traj_rmse']:.3f} | EndErr={best['traj_end_err']:.3f}")
    plt.legend()
    plt.show()

    # 8) Matriz de confusão (complementar)
    plt.figure()
    plt.imshow(cm)
    plt.title("Matriz de Confusão (Teste) - Complementar")
    plt.xlabel("Previsto")
    plt.ylabel("Real")
    ...
    # 9) Resumo final para colar no trabalho (console)
    print("\nResumo das métricas (rodadas):")
